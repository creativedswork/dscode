...
- `docs/prototypes/adapt-eval-to-subagents.html` — 自包含 CHIEF Multi-Agent Eval Dashboard 原型。确认使用 Agent Process Lanes 表达 Main/SubAgent 并发执行，以 `Application + 6 位 Agent ID` 展示真实责任主体，以跨 Agent 因果链区分根因、下游症状和恢复，并展示 Subtask → Agent → Step 三级回溯。
