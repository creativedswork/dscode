...
**Reason**: Stage prompts and a machine-readable manifest replace mutable phase-specific README instructions.

**Migration**: CHIEF Applications receive stage instructions in their Agent definition/task prompt and discover files through `manifest.json`.

### Requirement: Meta File

**Reason**: `meta.md` only describes Main Session tool counts and cannot express Multi-Agent evidence.

**Migration**: Store Session metadata and evidence counts in `manifest.json`.

### Requirement: Skeleton File

**Reason**: Hot/cold zone heuristics belong to the removed SCAN/ZOOM pipeline.

**Migration**: Use CHIEF Subtask graph and Oracle-guided backtracking outputs.

### Requirement: Signals File

**Reason**: Precomputed heuristic signal groups are not the source of CHIEF candidate localization.

**Migration**: Workers read normalized trajectory chunks and validated graph/oracle outputs.

### Requirement: Data Items File

**Reason**: File-operation frequency alone cannot represent cross-Agent data flow.

**Migration**: Use topology/data-edge indexes with concrete Actor and Step references.
