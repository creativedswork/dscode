...
When an MCP tool result is not valid JSON, the ToolCard SHALL render the result as a `.mcp-raw-block` with monospace font, pre-wrap whitespace, and 280px max-height with internal scroll. The `ToolCallEntry` type SHALL include optional `progress`, `progressTotal`, and `progressMessage` fields to support inline progress display.
...
### Requirement: ToolCallEntry extended with progress fields
The `ToolCallEntry` type in `src/ui/shared/types.ts` SHALL include optional `progress`, `progressTotal`, and `progressMessage` fields.

#### Scenario: Type definition
- **WHEN** a `ToolCallEntry` is created from a `tool_start` event
- **THEN** the entry SHALL have `progress?: number`, `progressTotal?: number`, `progressMessage?: string`
- **AND** all three fields SHALL default to `undefined`

#### Scenario: Progress fields populated by reducer
- **WHEN** the reducer processes a `tool_progress` event
- **THEN** the matching `ToolCallEntry` SHALL have `progress`, `progressTotal`, and `progressMessage` set from the event

### Requirement: conversationReducer handles tool_progress events
`conversationReducer` SHALL process `tool_progress` events to update in-progress `ToolCallEntry` objects within the last streaming assistant message.

#### Scenario: Reducer updates matching tool entry
- **WHEN** the reducer receives `{ type: "tool_progress", name: "mcp__search", progress: 50, total: 100 }`
- **AND** the last streaming message has a tool entry `{ name: "mcp__search", result: "" }`
- **THEN** that entry SHALL be updated with `progress: 50, progressTotal: 100`

#### Scenario: Reducer ignores progress for completed tools
- **WHEN** the reducer receives `tool_progress` for a tool that already has a non-empty `result`
- **THEN** the entry SHALL NOT be modified

#### Scenario: Reducer ignores progress for unknown tools
- **WHEN** the reducer receives `tool_progress` with a `name` not matching any tool in the last streaming message
- **THEN** the messages array SHALL return unchanged
