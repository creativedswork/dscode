...
The system SHALL preserve base64 image data from MCP tool results into `AgentToolResult.content` as `ImageContent` blocks, instead of replacing them with text placeholders.
...
Before entering the agent transcript or being sent to the UI, MCP tool result images SHALL be compressed using the existing `ImageCache` pipeline: max height 480px, JPEG quality 85, content-addressed storage.

#### Scenario: Large MCP image compressed
- **WHEN** an MCP tool returns an image with height > 480px
- **THEN** the system SHALL pass it through `ImageCache.put()` before adding to `AgentToolResult.content`
