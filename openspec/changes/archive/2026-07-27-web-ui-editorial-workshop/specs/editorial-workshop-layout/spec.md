...
Defines the spatial, typographic, and navigational structure of the editorial workshop layout — the app shell that replaces the current three-panel layout.
...
The sidebar SHALL be 220px wide and contain: a "Create" section label with a Sessions navigation item (with count badge), a "Capabilities" section label with MCP and Skills navigation items (each with count badge), and a Settings button in the footer area. Sections SHALL be visually separated by a 1px border. Clicking Sessions, MCP, Skills, or Settings SHALL open a 280px detail panel between the sidebar and main content.

#### Scenario: Sidebar navigation
- **WHEN** the app renders
- **THEN** the sidebar SHALL display "Create" as a section label above Sessions
- **AND** SHALL display "Capabilities" as a section label above MCP and Skills
- **AND** SHALL display Sessions, MCP, and Skills as navigation items with SVG icons and count badges
- **AND** the active nav item SHALL be highlighted with `--surface` background and `--text` color (not accent)
- **AND** a Settings button SHALL appear in the sidebar footer area

#### Scenario: Panel opens on click
- **WHEN** the user clicks a sidebar nav item (e.g., "MCP")
- **THEN** a 280px detail panel SHALL open between the sidebar and main content
...
- **THEN** the panel title SHALL use `font-family: var(--font-display)` at 15px
