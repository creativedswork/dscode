...
When the enabled Open Design integration requests auto-start, the system SHALL compile its validated configuration into a managed-service specification and delegate startup to ServiceSupervisor. Open Design-specific startup behavior MUST NOT be implemented directly in `src/core/main.ts`.
