...
**Reason**: Startup-time synchronization from environment variables into `~/.mcp.json` creates duplicate persistent sources of truth and performs an unrelated global write.

**Migration**: Configure `integrations.openDesign` in user or project `settings.json`. During the compatibility window, existing environment variables can supply missing integration fields for the current run, but dscode will not persist them automatically.

### Requirement: MCP entry path is local and absolute
**Reason**: The path contract moves from a generated persistent MCP entry to the in-memory Open Design integration contribution.

**Migration**: The derived in-memory `MCPServerConfig` continues to use an expanded local absolute daemon CLI path; no user action is required beyond migrating integration configuration.
