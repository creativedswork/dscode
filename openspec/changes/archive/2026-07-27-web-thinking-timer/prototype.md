...
- `docs/prototypes/web-thinking-timer-phase-label-cascade.html` — Phase label cascade 修复验证原型（左右对比：无 `data-collider` 的 phase label 在 cascade 后残留 vs 有 `data-collider="phase-label"` 后正确摧毁）。
...
6. **Phase label cascade**：`.phase-label` 添加 `data-collider="phase-label"`，让 TransitionCanvas cascade 动画能感知并摧毁 Thinking/Executing/Response 标签（修复 chat → dashboard 切换时的标签残留问题）。
