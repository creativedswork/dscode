...
- **Phase label cascade 修复**：TransitionCanvas 的 `buildRowList()` 仅选取 `[data-collider]` 叶子元素，而 `.phase-label`（`Thinking` / `Executing` / `Response`）缺少该属性，导致 chat → dashboard 切换的 cascade 动画中标签残留。为三处 `.phase-label` 添加 `data-collider="phase-label"`，使其被动画引擎感知并摧毁。
...
- `docs/prototypes/web-thinking-timer-phase-label-cascade.html`（explore 阶段确认的 phase label cascade 修复原型）
