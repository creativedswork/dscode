...
Define Open Design-owned typed configuration, compatibility inputs, enablement,
and managed-service preparation without leaking device-specific types into the
Application facade.
## Requirements
...
Open Design configuration SHALL remain internal to its integration owner.
HarnessAPI MUST NOT expose a mutable Open Design configuration object.

#### Scenario: UI reads general configuration

- **WHEN** TUI or Web queries the public configuration snapshot
- **THEN** no mutable Open Design configuration SHALL be exposed
- **AND** any future integration status UI SHALL use a generic read-only status query
