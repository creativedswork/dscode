...
MCP tool call abort support — AbortSignal贯穿 AgentTool.execute → MCPManager.buildAgentTool → MCPClient.callTool → MCPClient.request 全链路，允许用户在 MCP 工具执行期间通过 Ctrl+C 或 Stop 按钮中断操作。覆盖 stdio、streamable-http、SSE 三种传输协议。
