...
The system SHALL report progress to the UI during the 6-step analysis pipeline. Each step (0-6) SHALL display a message: "Step X/6: <step_description>...". Step 0 (deterministic parsing and stats) SHALL complete immediately and display initial statistics. Subsequent steps SHALL indicate when the LLM call is in progress.
