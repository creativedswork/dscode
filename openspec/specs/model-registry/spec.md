...
The system SHALL provide a model registry that can resolve a `Model<Api>` given a provider name and model ID. Resolution MUST try `pi-ai`'s built-in `getModel()` first, then fall back to provider-specific definitions registered in the registry.

#### Scenario: Resolve a pi-ai built-in model
- **WHEN** `resolveModel("deepseek", "deepseek-v4-flash")` is called
- **THEN** the system returns the model from `pi-ai`'s `getModel()` without consulting registry fallback

#### Scenario: Resolve a registry-defined model not in pi-ai
- **WHEN** `resolveModel("qwen", "qwen3.6-plus")` is called and pi-ai's `getModel` returns null
- **THEN** the system returns the model definition from the Qwen provider registry
...
Each supported provider's model definitions SHALL reside in its own file under `src/models/`, exporting only its model map, base URL constant, and factory function if needed.

#### Scenario: Qwen models are defined in qwen.ts
- **WHEN** a developer opens `src/models/qwen.ts`
- **THEN** they find `QWEN_MODELS`, `DASHSCOPE_BASE`, and `buildQwenModel` defined there, with no references to other providers
