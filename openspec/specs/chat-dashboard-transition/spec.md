...
The cascade animation SHALL implement a three-tier hierarchy of element interaction: (1) Active Strike — collidable elements with `data-collider` attributes receive full collision physics and type-specific destruction effects; (2) Passive Dissolution — timestamp metadata elements dissolve into particles on cluster proximity without collision; (3) Preserved — thinking blocks remain completely untouched and visible throughout.
