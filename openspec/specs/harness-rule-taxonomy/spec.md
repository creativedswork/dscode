# harness-rule-taxonomy Specification

## Purpose

Pre-defined rule catalog mapping agent behavior patterns to dscode configuration layers. Defines the `HarnessRule` type, `RuleCategory` enum mapped to system prompt sections, `RulePattern` for machine-detectable triggers, `RuleEvidence` for cross-session accumulation, and `RuleSuggestion` for concrete configuration modifications.
