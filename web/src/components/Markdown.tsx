...
export function Markdown({ children, className = "", isStreaming = false }: MarkdownProps) {
  const components = React.useMemo(() => ({
    p: ({ children }: React.PropsWithChildren) => (
      <p data-collider="text-block">{children}</p>
    ),
    li: ({ children }: React.PropsWithChildren) => (
      <li data-collider="text-block">{children}</li>
    ),
    blockquote: ({ children }: React.PropsWithChildren) => (
      <blockquote data-collider="text-block">{children}</blockquote>
    ),
    h1: ({ children }: React.PropsWithChildren) => (
      <h1 data-collider="text-block">{children}</h1>
    ),
    h2: ({ children }: React.PropsWithChildren) => (
      <h2 data-collider="text-block">{children}</h2>
    ),
    h3: ({ children }: React.PropsWithChildren) => (
      <h3 data-collider="text-block">{children}</h3>
    ),
    h4: ({ children }: React.PropsWithChildren) => (
      <h4 data-collider="text-block">{children}</h4>
    ),
