...
<td width="33%" valign="top">
  <strong>DeepSeek 原生优化</strong><br />
  围绕 DeepSeek 模型能力设计，不做多模型折中层，配置、推理模式和体验都为 DeepSeek 优化。
</td>
<td width="33%" valign="top">
  <strong>智能体集成</strong><br />
...
`/config` 命令写入的配置会持久化到 `~/.dscode/config.json`，后续启动无需重复设置。其中 `cwd` 会记录本次启动对应的项目目录；只有在你手动执行 `/config cwd <path>` 时，才会对当前项目复用该目录。声明式配置单独放在 `settings.json`：用户级 `~/.dscode/settings.json`，项目级 `<project>/.dscode/settings.json`。
